...
const getQuizCategories = async (): Promise<Category[]> => {
  try {
    ...
    if (result.status !== 200) {
      throw new Error(
        `Faild to fetch categories. Why? status -> ${result.statusText} | ${result.status}`
      );
    } else return result.data["trivia_categories"];
  } catch (e) {
    if (isAxiosError(e)) {
...
export const useQuizCategories = () => {
  const { isLoading, isError, data, error, ...rest } = useQuery(
    ["quizCategories"],
    getQuizCategories
  );
